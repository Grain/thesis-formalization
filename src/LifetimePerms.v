...
Section LifetimePerms.
  ...
  Definition lifetime_Perms :=
    join_Perms (fun Q => exists n : nat, Q = singleton_Perms (lifetime_perm n)).
  ...
  Lemma nonLifetime_top : nonLifetime top_perm.
  Proof.
    repeat intro. symmetry. apply separate_top.
  Qed.

  Lemma nonLifetime_lte p q :
    nonLifetime p -> p <= q -> nonLifetime q.
  Proof.
    repeat intro. symmetry. eapply separate_upwards_closed; eauto. symmetry. apply H.
  Qed.
  ...
  Lemma nonLifetime_Perms_bottom :
    nonLifetime_Perms top_Perms.
  Proof.
    repeat intro. exists top_perm. split; [| split; [| split; [| split; [| split]]]]; auto.
  ...
  Proof.
    ...
    assert (p' ⊥ q').
    {
      eapply separate_upwards_closed. 2: apply Hqq'.
      symmetry. eapply separate_upwards_closed. 2: apply Hpp'.
      symmetry. auto.
    }
  ...
  Definition when_Perms l P : Perms :=
    join_Perms (fun R => exists p, p ∈ P /\ R = singleton_Perms (when l p)).
  ...
  Proof.
    ...
    - apply when_preserves_sep; auto.
    - etransitivity; eauto.
      etransitivity. apply when_monotone; eauto.
  ...
  Lemma lfinished_sep_conj_perm_dist l p q :
    lfinished l (p ** q) <= lfinished l p ** lfinished l q.
  Proof.
    split.
...
*)
  ...
  Lemma sep_step_beginLifetime n :
    sep_step (lifetime_perm n)
             (owned n top_perm nonLifetime_top ** lifetime_perm (n + 1)).
  Proof.
    apply sep_step_rg.
  ...
  Proof.
    ...
    - apply sep_conj_Perms_perm.
      + cbn. exists top_perm, top_perm. eexists.
        { intros. symmetry. apply separate_top. }
