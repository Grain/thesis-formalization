...
(** * Helper lemmas and definitions about [no_errors] and [bisim] **)
...
Lemma bisim_tau_L {S1 S2 R1 R2} p Q t1 s1 t2 s2 :
  bisim p Q (Tau t1) s1 t2 s2 -> @bisim S1 S2 R1 R2 p Q t1 s1 t2 s2.
Proof.
  intro; punfold H.
  dependent induction H; pfold; try econstructor; eauto.
  + specialize (IHbisim_gen t1 JMeq_refl eq_refl).
    punfold IHbisim_gen.
...
Lemma bisim_tau_R {S1 S2 R1 R2} p Q t1 s1 t2 s2 :
  bisim p Q t1 s1 (Tau t2) s2 -> @bisim S1 S2 R1 R2 p Q t1 s1 t2 s2.
Proof.
  intro; punfold H.
  dependent induction H; pfold; try econstructor; eauto.
  + specialize (IHbisim_gen t2 JMeq_refl eq_refl).
    punfold IHbisim_gen.
  + pclearbot; punfold H0.
  + specialize (IHbisim_gen t2 JMeq_refl eq_refl).
    punfold IHbisim_gen.
...
Lemma bisim_inv_tau_L {S1 S2 R1 R2} p Q t1 s1 t2 s2 (ne2 : no_errors s2 t2) :
  bisim p Q t1 s1 t2 s2 -> @bisim S1 S2 R1 R2 p Q (Tau t1) s1 t2 s2.
Proof.
  intro; punfold H; pfold; econstructor; eauto.
  apply bisim_gen_pre in H; destruct H; eauto.
  rewrite H in ne2; punfold ne2; inv ne2.
Qed.

Lemma bisim_inv_tau_R {S1 S2 R1 R2} p Q t1 s1 t2 s2 (ne2 : no_errors s2 t2) :
  bisim p Q t1 s1 t2 s2 -> @bisim S1 S2 R1 R2 p Q t1 s1 (Tau t2) s2.
Proof.
  intro; punfold H; pfold; econstructor; eauto.
  apply bisim_gen_pre in H; destruct H; eauto.
  rewrite H in ne2; punfold ne2; inv ne2.
...
Definition bisim_ex {S1 S2 R1 R2} (p:@perm (S1*S2)) (Q: R1 -> R2 -> Perms) t1 s1 t2 s2 :=
  exists p', sep_step p p' /\ bisim p' Q t1 s1 t2 s2.

Definition bisim_to_bisim_ex {S1 S2 R1 R2} p Q t1 s1 t2 s2 :
  bisim p Q t1 s1 t2 s2 -> @bisim_ex S1 S2 R1 R2 p Q t1 s1 t2 s2.
...
(** * Helper lemmas and definitions relating [bisim] and paths *)
(** ** [bisim_path_r], [exists_bisim_path_r], and [bisim_impl_path_r]  **)

(** An impressionistic picture of [bisim_impl_path_r], where the solid line is
   assumed, the dotted lines are shown to exist, and all the center lines
   represent [bisim_ex].
<<
   (t2,s2) ⋯⋯ (t4,s4)      --
                  ⋮ step      | all dotted lines:
               (ti,si)       | bisim_path_r ((t1,s1), (t3,s3))
            ⋰    ⋮ step      |               ((t2,s2), (t4,s4))
   (t1,s1) --- (t3,s3)      --
        bisim_ex
>>
   In words, this picture states that if [bisim_ex t1 s1 t3 s3] then there
   exists some [(t4,s4)] which satisfies [bisim_ex t2 s2 t4 s4] and for which
   there exists a finite path from [(t3,s3)], where each intermediate point
   along the path satisfies [bisim_ex t1 s1 ti si].
*)

(** [bisim_path_r ((t1,s1), (t3,s3)) ((t2,s2), (t4,s4))] represents all the
   dotted lines in the picture above. *)
Inductive bisim_path_r {S1 S2 R1 R2} p Q :
...
| ex_path_r t1 s1 t2 s2 t3 s3 t4 s4 n (ts : Vector.t (CompM S2 R2 * S2) n) :
    ...
    (forall i, bisim_ex p Q t1 s1 (fst ts[@i]) (snd ts[@i]) /\
               guar p (s1, s3) (s1, snd ts[@i])) ->
    bisim_ex p Q t2 s2 t4 s4 -> guar p (s1, s3) (s2, s4) ->
    bisim_path_r p Q ((t1,s1),(t3,s3)) ((t2,s2),(t4,s4)).
Arguments ex_path_r {S1 S2 R1 R2 p Q t1 s1 t2 s2 t3 s3 t4 s4} n ts.

(** Like [bisim_path_r] but with the endpoints on the right existentially
   quantified and the arguments curried. *)
Definition exists_bisim_path_r {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :=
  exists t4 s4, @bisim_path_r S1 S2 R1 R2 p Q ((t1,s1),(t3,s3)) ((t2,s2),(t4,s4)).

Definition bisim_impl_path_r {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :=
  bisim_ex p Q t1 s1 t3 s3 -> @exists_bisim_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 t3 s3.


(** ** [bisim_step_l] *)

(** We start with some lemmas about [bisim_path_r] and [exists_bisim_path_r]. *)

Lemma exists_path_r_tau_R {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :
  bisim_ex p Q t1 s1 t3 s3 ->
  exists_bisim_path_r p Q t1 s1 t2 s2 t3 s3 ->
  @exists_bisim_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 (Tau t3) s3.
Proof.
...
Lemma exists_path_r_modify_R {S1 S2 R1 R2} p p' Q t1 s1 t2 s2 f k s3 :
  ...
  sep_step p p' ->
  bisim_gen (upaco6 bisim_gen bot6) p' Q t1 s1 (k s3) (f s3) ->
  exists_bisim_path_r p Q t1 s1 t2 s2 (k s3) (f s3) ->
  @exists_bisim_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 (vis (Modify f) k) s3.
Proof.
  intros ? ? Hb [t4 [s4 H]]; dependent destruction H.
...
Lemma exists_path_r_choice_R {S1 S2 R1 R2} b p p' Q t1 s1 t2 s2 k s3 :
  sep_step p p' ->
  bisim_gen (upaco6 bisim_gen bot6) p' Q t1 s1 (k b) s3 ->
  exists_bisim_path_r p Q t1 s1 t2 s2 (k b) s3 ->
  @exists_bisim_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 (vis Or k) s3.
Proof.
  intros ? Hb [t4 [s4 H]]; dependent destruction H.
...
(** Next we prove [bisim_impl_path_r] for [step]. *)

Lemma tau_step_bisim_impl_path_r {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :
  no_errors s3 t3 ->
  tau_step (t1,s1) (t2,s2) ->
  @bisim_impl_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 t3 s3.
Proof.
...
Lemma modify_step_bisim_impl_path_r {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :
  no_errors s3 t3 ->
...
Lemma choice_step_bisim_impl_path_r {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :
  no_errors s3 t3 ->
...
Lemma bisim_step_l {S1 S2 R1 R2} p Q t1 s1 t2 s2 t3 s3 :
  no_errors s3 t3 ->
  step (t1,s1) (t2,s2) ->
  @bisim_impl_path_r S1 S2 R1 R2 p Q t1 s1 t2 s2 t3 s3.
Proof.
  intros ne3 H; dependent destruction H.
  - apply tau_step_bisim_impl_path_r, tau_step_Tau; eauto.
  - apply choice_step_bisim_impl_path_r, choice_step_Choice; eauto.
  - apply modify_step_bisim_impl_path_r, modify_step_Modify; eauto.
Qed.


(** ** [bisim_path_l], [exists_bisim_path_l], and [bisim_impl_path_l]  **)

(** An impressionistic picture of [bisim_impl_path_l], analogous to that
    for [bisim_impl_path_r]
<<
      (t2,s2) ⋯⋯ (t4,s4)   --
   step  ⋮                     | all dotted lines:
      (ti,si)                 | bisim_path_l ((t1,s1), (t3,s3))
   step  ⋮     ⋱              |               ((t2,s2), (t4,s4))
      (t1,s1) --- (t3,s3)   --
           bisim_ex
>>
...
(** [bisim_path_l ((t1,s1), (t3,s3)) ((t2,s2), (t4,s4))] represents all the
   dotted lines in the picture above. *)
Inductive bisim_path_l {S1 S2 R1 R2} p Q :
  relation ((CompM S1 R1 * S1) * (CompM S2 R2 * S2)) :=
| ex_path_l t1 s1 t2 s2 t3 s3 t4 s4 n (ts : Vector.t (CompM S1 R1 * S1) n) :
    is_finite_path n (t1,s1) ts (t2,s2) ->
    (forall i, bisim_ex p Q (fst ts[@i]) (snd ts[@i]) t3 s3 /\
               guar p (s1, s3) (snd ts[@i], s3)) ->
    bisim_ex p Q t2 s2 t4 s4 -> guar p (s1, s3) (s2, s4) ->
    bisim_path_l p Q ((t1,s1),(t3,s3)) ((t2,s2),(t4,s4)).
Arguments ex_path_l {S1 S2 R1 R2 p Q t1 s1 t2 s2 t3 s3 t4 s4} n ts.

(** Like [bisim_path_l] but with the endpoints on the left existentially
   quantified and the arguments curried. *)
Definition exists_bisim_path_l {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :=
  exists t2 s2, @bisim_path_l S1 S2 R1 R2 p Q ((t1,s1),(t3,s3)) ((t2,s2),(t4,s4)).

Definition bisim_impl_path_l {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :=
  bisim_ex p Q t1 s1 t3 s3 -> @exists_bisim_path_l S1 S2 R1 R2 p Q t1 s1 t3 s3 t4 s4.


(** ** [bisim_step_r] *)

(** We start with some lemmas about [bisim_path_l] and [exists_bisim_path_l].
   The proofs here are mostly identical to those for [bisim_step_l] above. *)

Lemma exists_path_l_tau_L {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :
  bisim_ex p Q t1 s1 t3 s3 ->
  exists_bisim_path_l p Q t1 s1 t3 s3 t4 s4 ->
  @exists_bisim_path_l S1 S2 R1 R2 p Q (Tau t1) s1 t3 s3 t4 s4.
Proof.
...
Lemma exists_path_l_modify_L {S1 S2 R1 R2} p p' Q f k s1 t3 s3 t4 s4 :
  ...
  sep_step p p' ->
  bisim_gen (upaco6 bisim_gen bot6) p' Q (k s1) (f s1) t3 s3 ->
  exists_bisim_path_l p Q (k s1) (f s1) t3 s3 t4 s4 ->
  @exists_bisim_path_l S1 S2 R1 R2 p Q (vis (Modify f) k) s1 t3 s3 t4 s4.
Proof.
  intros ? ? Hb [t2 [s2 H]]; dependent destruction H.
...
Lemma exists_path_l_choice_L {S1 S2 R1 R2} b p p' Q k s1 t3 s3 t4 s4 :
  sep_step p p' ->
  bisim_gen (upaco6 bisim_gen bot6) p' Q (k b) s1 t3 s3 ->
  exists_bisim_path_l p Q (k b) s1 t3 s3 t4 s4 ->
  @exists_bisim_path_l S1 S2 R1 R2 p Q (vis Or k) s1 t3 s3 t4 s4.
Proof.
  intros ? Hb [t2 [s2 H]]; dependent destruction H.
...
(** Next we prove [bisim_impl_path_r] for [step]. Again, the proofs here are
   mostly identical to those for [bisim_step_l] above. *)

Lemma tau_step_bisim_impl_path_l {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :
  tau_step (t3,s3) (t4,s4) ->
  @bisim_impl_path_l S1 S2 R1 R2 p Q t1 s1 t3 s3 t4 s4.
Proof.
...
Lemma modify_step_bisim_impl_path_l {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :
...
Lemma choice_step_bisim_impl_path_l {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :
...
Lemma bisim_step_r {S1 S2 R1 R2} p Q t1 s1 t3 s3 t4 s4 :
  step (t3,s3) (t4,s4) ->
  @bisim_impl_path_l S1 S2 R1 R2 p Q t1 s1 t3 s3 t4 s4.
Proof.
  intros H; dependent destruction H.
  - apply tau_step_bisim_impl_path_l, tau_step_Tau; eauto.
  - apply choice_step_bisim_impl_path_l, choice_step_Choice; eauto.
  - apply modify_step_bisim_impl_path_l, modify_step_Modify; eauto.
Qed.
...
Definition eq_sat_sep_bisim {S1 S2 R1 R2} (q:@perm (S1*S2))
  (P1:TPred S1 R1) (P2:TPred S2 R2) :=
  forall p Q t1 s1 t2 s2, pre q (s1,s2) -> separate p q ->
    bisim p Q t1 s1 t2 s2 -> no_errors s2 t2 ->
    (P1 (t1,s1) <-> P2 (t2,s2)).


(** * [eq_sat_sep_bisim] for state predicates **)
...
Lemma eq_sat_state_preds {S1 S2 R1 R2} q (P1 : S1 -> Prop) (P2 : S2 -> Prop)
  : q_similar q P1 P2 ->
    eq_sat_sep_bisim q (state_pred R1 P1) (state_pred R2 P2).
Proof.
  unfold eq_sat_sep_bisim; intros.
  apply H; assumption.
Qed.


(** * [eq_sat_sep_bisim] for logical connectives **)

Lemma eq_sat_and {S1 S2 R1 R2} q (TP1 TP1' : TPred S1 R1) (TP2 TP2' : TPred S2 R2)
  : eq_sat_sep_bisim q TP1 TP2 -> eq_sat_sep_bisim q TP1' TP2' ->
    eq_sat_sep_bisim q (fun ts => TP1 ts /\ TP1' ts) (fun ts => TP2 ts /\ TP2' ts).
Proof.
...
Lemma eq_sat_EU {S1 S2 R1 R2} q (P1 P1' : TPred S1 R1) (P2 P2' : TPred S2 R2) :
    eq_sat_sep_bisim q P1 P2 -> eq_sat_sep_bisim q P1' P2' ->
    eq_sat_sep_bisim q (EU P1 P1') (EU P2 P2').
Proof.
  ...
  - revert p t2 s2 H H0 H1 H2; dependent induction H3; intros.
    ...
    + destruct ts1.
      pose proof (bisim_to_bisim_ex _ _ _ _ _ _ H4).
      pose proof (bisim_step_l _ _ _ _ _ _ _ _ H5 H0 H6).
  ...
  - revert p t1 s1 H H0 H1 H2; dependent induction H3; intros.
    ...
    + destruct ts1.
      pose proof (bisim_to_bisim_ex _ _ _ _ _ _ H4).
      pose proof (bisim_step_r _ _ _ _ _ _ _ _ H0 H6).
...
Lemma eq_sat_AG {S1 S2 R1 R2} q (P1 : TPred S1 R1) (P2 : TPred S2 R2) :
    eq_sat_sep_bisim q P1 P2 ->
    eq_sat_sep_bisim q (AG P1) (AG P2).
Proof.
  ...
  - revert p t1 s1 t2 s2 H H0 H1 H2 H3; pcofix CIH; intros.
    ...
    + punfold H4; destruct H4; eauto.
      eapply eq_sat_Ps; eauto.
    + intros; destruct ts1.
      apply bisim_to_bisim_ex in H2.
      pose proof (bisim_step_r _ _ _ _ _ _ _ _ H H2).
  ...
  - revert p t1 s1 t2 s2 H H0 H1 H2 H3; pcofix CIH; intros.
    ...
    + intros; destruct ts1.
      apply bisim_to_bisim_ex in H2.
      pose proof (bisim_step_l _ _ _ _ _ _ _ _ H3 H H2).
...
Lemma eq_sat_EG {S1 S2 R1 R2} q (P1 : TPred S1 R1) (P2 : TPred S2 R2) :
    eq_sat_sep_bisim q P1 P2 ->
    eq_sat_sep_bisim q (EG P1) (EG P2).
Proof.
  intro eq_sat_Ps; split; intros.
  - revert p t1 s1 t2 s2 H H0 H1 H2 H3; pcofix CIH; intros.
...
Lemma eq_sat_AU {S1 S2 R1 R2} q (P1 P1' : TPred S1 R1) (P2 P2' : TPred S2 R2) :
    eq_sat_sep_bisim q P1 P2 -> eq_sat_sep_bisim q P1' P2' ->
    eq_sat_sep_bisim q (AU P1 P1') (AU P2 P2').
Proof.
  intros eq_sat_Ps eq_sat_P's; split; intros.
  - revert p t2 s2 H H0 H1 H2; dependent induction H3; intros.
...
Lemma tpsim_implies_eq_sat_sep_bisim {S1 S2 R1 R2} q TP1 TP2:
  CTLsim q TP1 TP2 -> @eq_sat_sep_bisim S1 S2 R1 R2 q (CTLsats TP1) (CTLsats TP2).
Proof.
...
Theorem bisim_preserves_tpreds {S1 R1 S2 R2} p q Q t1 s1 t2 s2 TP1 TP2:
  @bisim S1 R1 S2 R2 p Q t1 s1 t2 s2 -> no_errors s2 t2 ->
...
Proof.
  ...
  eapply (tpsim_implies_eq_sat_sep_bisim q TP1 TP2 tp_sim); eassumption.
Qed.
